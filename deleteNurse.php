...
<?php
	...
	{ 
		...
		//$query_sel = "SELECT  patientId,Title,lastName, firstName , dateOfBirth, gender , streetAddress, addressLine2, city, country, email, homePhone, workPhone, cellPhone, height, weight, age, allergy, bloodType,bmi,dateadded FROM Patient  LIMIT $limit OFFSET $offset";
			$query_selectdoctor = "SELECT nurseId,`lastName`, `firstName`, `city`, `country`, `email`, `homePhone`, `workPhone`, `cellPhone`, `qualification`, `photolocation`,profileInfo FROM `medical`.`nurse2`";
...
echo"<table style='width:90%' class='table table-striped table-hover table-bordered' >
    ...
    while($row = $result->fetch_assoc()) {
		...
		<input name ='nurseId'type='hidden' value='" .$row['nurseId']. "'class='form-control'/>
		...
		<td><button name='btn_DeleteNurse' class='btn btn-danger btn-md form-group' type='submit'>Delete Nurse</button></td>
	...
	echo"</table>";
	echo"</center>";
	if(isset($_POST['btn_DeleteNurse']))
					{
						$nurseId =$_POST['nurseId'];
					$query_deleteNurse = "DELETE FROM `nurse2` WHERE `nurse2`.`nurseId` =$nurseId ";
					
			//mysqli_query($con, $query_sel);
			mysqli_query($con, $query_deleteNurse);
			//echo "<script> location.href = 'deletenurse.php'</script> 
						echo "
